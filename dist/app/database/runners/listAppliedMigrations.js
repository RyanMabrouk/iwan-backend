...
async function listAppliedMigrations() {
    ...
    return results.map((migration) => migration.name);
}
async function displayAppliedMigrations() {
    try {
        const appliedMigrations = await listAppliedMigrations();
        console.log('Applied migrations:');
        appliedMigrations.forEach((migrationName) => console.log('-', migrationName));
    }
    catch (error) {
...
//# sourceMappingURL=listAppliedMigrations.js.map